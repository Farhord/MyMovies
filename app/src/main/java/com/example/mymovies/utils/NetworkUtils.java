...
public class NetworkUtils {
    ...
    private static final String BASE_MOVIE_URL = "https://api.themoviedb.org/3/movie/%s";
    ...
    public static URL buildURL(int sortBy, int page, String lang) {
        ...
        } else methodOfSort = SORT_BY_VOTE_AVERAGE;
    ...
    public static URL buildMovieURL(int movieId, String lang) {
        Uri uri = Uri.parse(String.format(BASE_MOVIE_URL, movieId)).buildUpon()
                .appendQueryParameter(PARAMS_API_KEY, API_KEY)
                .appendQueryParameter(PARAMS_LANGUAGE, lang)
                .build();
        try {
    ...
    public static URL buildVideosURL(int movieId, String lang) {
        Uri uri = Uri.parse(String.format(BASE_VIDEOS_URL, movieId)).buildUpon()
                .appendQueryParameter(PARAMS_API_KEY, API_KEY)
                .appendQueryParameter(PARAMS_LANGUAGE, lang)
                .build();
        try {
    ...
    public static URL buildReviewsURL(int movieId) {
        Uri uri = Uri.parse(String.format(BASE_REVIEWS_URL, movieId)).buildUpon()
                .appendQueryParameter(PARAMS_API_KEY, API_KEY)
                .build();
        try {
            return new URL(uri.toString());
    ...
    public static JSONObject getJSONForMovie(int movieId, String lang) {
        JSONObject jsonObject = null;
        URL url = buildMovieURL(movieId, lang);
        try {
            jsonObject = new JSONLoadTask().execute(url).get();
        } catch (ExecutionException e) {
            e.printStackTrace();
    ...
    public static JSONObject getJSONForVideos(int movieId, String lang) {
        JSONObject jsonObject = null;
        URL url = buildVideosURL(movieId, lang);
        try {
            jsonObject = new JSONLoadTask().execute(url).get();
        } catch (ExecutionException e) {
            e.printStackTrace();
